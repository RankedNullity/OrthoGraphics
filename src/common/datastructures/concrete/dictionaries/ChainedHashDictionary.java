...
public class ChainedHashDictionary<K, V> implements IDictionary<K, V> {
	...
	private static class ChainedIterator<K, V> implements Iterator<KVPair<K, V>> {
		...
		/**
		 * Returns the index of the next non-null bucket. If no more exist, then returns
		 * chains.size();
		 * 
		 * @return
		 */
		private void updateIndex() {
			...
			while (nextUsefulIndex < chains.length
					&& (chains[nextUsefulIndex] == null || !chains[nextUsefulIndex].iterator().hasNext())) {
				nextUsefulIndex++;
			}
		}
